...
\State $k \gets 0$
\State $x^{(k)} \gets x^{(0)}$
\While{$k < max_{iter}$ and $||x^{(k+1)} - x^{(k)}||2 \geq \epsilon$}
\State $k \gets k+1$
\For{$i = 1$ to $n$}
\State $s \gets \sum{j=1}^{i-1} a_{ij}x_j^{(k+1)} + \sum_{j=i+1}^{n} a_{ij}x_j^{(k)}$
\State $x_i^{(k+1)} \gets (1-\omega) x_i^{(k)} + \frac{\omega}{a_{ii}} (b_i - s)$
\EndFor
\EndWhile
